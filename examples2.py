...
def create_example_network() -> RoadNetwork:
    ...
    # Skrzyżowania
    A = network.add_intersection("A Grunwaldzka x Droga Zielona", 0, 0)
    B = network.add_intersection("B Droga Zielona x Gospody", 180, 0)
    C = network.add_intersection("C Grunwaldzka x Pomorska", 0, 150)
    D = network.add_intersection("D Pomorska x Gospody x Chłopska", 200, 125)
    E = network.add_intersection("E Jelitkowo", 325, 100)
    F = network.add_intersection("F Grunwaldzka x Piastowska x Rybińskiego", 0, 200)
    G = network.add_intersection("G Piastowska x Chłopska", 220, 175)
    H = network.add_intersection("H Piastowsaka E", 275, 170)
    I = network.add_intersection("I Piastowska x Droszyńskiego", 50, 195)
    J = network.add_intersection("J Grunwaldzka x Kołobrzeska", 30, 400)
    K = network.add_intersection("K Kołobrzeska x Rzeczypospolitej", 250, 370)
    L = network.add_intersection("L Chłopska x Jagiellońska", 230, 235)
    M = network.add_intersection("M Chłopska x Komorowskiego", 240, 295)
    N = network.add_intersection("N Droszyńskiego x Komorowskiego", 75, 275)
    O = network.add_intersection("O Obrońców Wybrzerza E", 460, 280)
    P = network.add_intersection("P Jagiellońska E", 450, 220)
    R = network.add_intersection("R Kołobrzeska E", 470, 355)
    S = network.add_intersection("S Rybińskiego W", -50, 200)
